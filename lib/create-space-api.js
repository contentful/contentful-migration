...
export default function createSpaceApi ({
   ...
   * @param {object} data.fields.file.[LOCALE].uploadFrom - Can be a string, an ArrayBuffer, or a Stream.
   * @return {Promise<Asset.Asset>} Promise for the newly created Asset
   * @example
   * space.createAsset({
   *   fields: {
   ...
   *          contentType: 'image/jpeg',
   *          fileName: 'filename_english.jpg',
   *          uploadFrom: readFileSync('path/to/filename_english.jpg')
   *       },
   *       'de-DE': {
   *          contentType: 'image/svg',
   *          fileName: 'filename_german.svg',
   *          uploadFrom: '<svg><path fill="red" d="M50 50h150v50H50z"/></svg>'
   *       }
   *     }
   *   }
   * })
   * .then(e => console.log(e))
   */
  async function createAssetFromFiles (data) {
    const { file } = data.fields
    const uploads = await Promise.all(
      Object.keys(file)
        .map((locale) => _uploadFile(locale, file[locale]))
    )
    data.fields.file = uploads.reduce((fieldsData, upload) => {
      const { locale, id, fileName, contentType } = upload
      fieldsData[locale] = {
        contentType,
        fileName,
        upload: {
  ...
  async function _uploadFile (locale, data) {
    const { file, fileName, contentType } = data
    try {
      const uploadResponse = await httpUpload.post('uploads', file, {
        headers: {
          'Content-Type': 'application/octet-stream'
        }
      })

      const { id } = uploadResponse.data.sys
